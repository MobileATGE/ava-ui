...
class SpeechSDKHelper {
  ...
  async start() {
    ...
    recognizer.recognized = function(s, e) {
      console.log('recognized isPlayerActive=', isPlayerActive);
      console.log('recognized stopChatInput=', stopChatInput);
      console.log('recognized e.result.text=', e.result.text);
      if (stopChatInput) {
        stopChatInput = false;
        return;
      }
      console.log('recognized stopChatInput 2=', stopChatInput);
      if (e.result.reason !== SpeechSDK.ResultReason.NoMatch) {
        document.querySelector(".sc-user-input--text").textContent = e.result.text;
      }
    };
